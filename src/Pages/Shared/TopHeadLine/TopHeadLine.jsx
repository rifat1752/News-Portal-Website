...
const TopHeadLine = () => {
  ...
  const { category, setCategory, pageIndex, setPageIndex } = useContext(AuthContext);
    ...
    useEffect(() => {
      ...
      console.log("top headline",headLines)
      
    return (
        <div className="border-l-2 border-[#ffa589] flex flex-col w-full overflow-hidden pl-4   ">
            <button className=" text-white text-poppins text-center h-10 bg-[#f8825e]">Top HeadLine</button>
            <div className="flex flex-col">
           
                {
                    headLines.articles?.slice(0,50).map((headLine, index)=> headLine.title !== "[Removed]" && (
